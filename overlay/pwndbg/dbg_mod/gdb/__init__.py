...
class GDBProcess(pwndbg.dbg_mod.Process):
    ...
    def module_section_locations(self) -> list[tuple[int, int, str, str]]:
        ...
        from pwndbg.aglib.container_rootfs import normalize_shared_library_path_for_current_inferior
        ...
        for section in pwndbg.gdblib.info.sections():
            result.append(
                (
                    ...
                    section.size,
                    section.section,
                    normalize_shared_library_path_for_current_inferior(section.objfile),
                )
            )
    ...
    def main_module_name(self) -> str | None:
        ...
        if self.alive() and not pwndbg.aglib.qemu.is_qemu_kernel():
            ...
            return normalize_shared_library_path_for_current_inferior(
                exe[exe.find("exe = '") + 7 : exe.rfind("'")]
            )
        filename = gdb.current_progspace().filename
        if filename is None:
            return None
        return normalize_shared_library_path_for_current_inferior(filename)
