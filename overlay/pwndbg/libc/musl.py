...
def has_internal_symbols(libc_filepath: str) -> bool:
    ...
    try:
        return (
            pwndbg.aglib.symbol.lookup_symbol("c_messages", objfile_endswith=libc_filepath)
            is not None
        )
    except Exception:
        return False
...
def verify_libc_candidate(mapping_name: str) -> bool:
    ...
    try:
        if (
            ...
            and pwndbg.aglib.symbol.lookup_symbol("__freadahead", objfile_endswith=mapping_name)
            is None
        ):
            return False
    except Exception:
