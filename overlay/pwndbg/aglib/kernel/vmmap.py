...
kernel_vmmap_cache_mode = pwndbg.config.add_param(
    "kernel-vmmap-cache",
    "auto",
    "the cache policy for kernel vmmap generation",
    help_docstring="""\
Values explained:

+ `auto` - persist kernel vmmap results across stops unless a task-specific PGD was selected with `kcurrent --set`
+ `per-stop` - recompute kernel vmmap on every stop
+ `persistent` - reuse the generated kernel vmmap across stops until start/objfile or manual invalidation

This helps when kernel vmmap generation is noticeably slow during QEMU kernel debugging.
""",
    param_class=pwndbg.lib.config.PARAM_ENUM,
    enum_sequence=["auto", "per-stop", "persistent"],
)


def _kernel_vmmap_pages_uncached() -> tuple[Page, ...]:
...
@pwndbg.lib.cache.cache_until("stop")
def _kernel_vmmap_pages_per_stop() -> tuple[Page, ...]:
    return _kernel_vmmap_pages_uncached()


@pwndbg.lib.cache.cache_until("start", "objfile")
def _kernel_vmmap_pages_persistent() -> tuple[Page, ...]:
    return _kernel_vmmap_pages_uncached()


def clear_kernel_vmmap_caches(clear_stop_caches: bool = True) -> None:
    pwndbg.lib.cache.clear_function_cache(_kernel_vmmap_pages_per_stop)
    pwndbg.lib.cache.clear_function_cache(_kernel_vmmap_pages_persistent)
    if clear_stop_caches:
        pwndbg.lib.cache.clear_cache(CacheUntilEvent.STOP)


def _use_persistent_kernel_vmmap_cache() -> bool:
    mode = str(kernel_vmmap_cache_mode)
    if mode == "persistent":
        return True
    if mode == "per-stop":
        return False
    ...
    assert mode == "auto"
    return pwndbg.commands.kcurrent.KCURRENT_PGD is None


def kernel_vmmap_pages() -> tuple[Page, ...]:
    if _use_persistent_kernel_vmmap_cache():
        return _kernel_vmmap_pages_persistent()
    return _kernel_vmmap_pages_per_stop()


@pwndbg.config.trigger(kernel_vmmap_mode)
@pwndbg.config.trigger(kernel_vmmap_cache_mode)
def _invalidate_kernel_vmmap_on_config_change() -> None:
    clear_kernel_vmmap_caches()
