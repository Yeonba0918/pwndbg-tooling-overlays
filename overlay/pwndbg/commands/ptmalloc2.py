...
from pwndbg.aglib.heap.ptmalloc import diff_heap_snapshots
from pwndbg.aglib.heap.ptmalloc import capture_heap_state_snapshot
from pwndbg.aglib.heap.ptmalloc import get_heap_snapshot
from pwndbg.aglib.heap.ptmalloc import list_heap_snapshots
from pwndbg.aglib.heap.ptmalloc import store_heap_snapshot
...
def _format_snapshot_field(name: str, value) -> str:
    if isinstance(value, int):
        return f"{name}={value:#x}"
    return f"{name}={value}"


def _format_chunk_delta(delta) -> str:
    if delta.before is None and delta.after is not None:
        return message.success(f"+ chunk {delta.address:#x} size={delta.after.real_size:#x}")
    if delta.after is None and delta.before is not None:
        return message.error(f"- chunk {delta.address:#x} size={delta.before.real_size:#x}")
    ...
    for field_name in (
        ...
        before_value = getattr(delta.before, field_name)
        after_value = getattr(delta.after, field_name)
        if before_value != after_value:
            changed.append(f"{field_name}: {before_value!r} -> {after_value!r}")
    return message.notice(f"* chunk {delta.address:#x}: " + ", ".join(changed))


def _format_bin_delta(delta) -> str:
    prefix = f"{delta.bin_type}[{delta.key}]"
    if delta.before is None and delta.after is not None:
        return message.success(f"+ {prefix}: fd={list(delta.after.fd_chain)!r}")
    if delta.after is None and delta.before is not None:
        return message.error(f"- {prefix}: fd={list(delta.before.fd_chain)!r}")

    assert delta.before is not None and delta.after is not None
    changes = []
    if delta.before.fd_chain != delta.after.fd_chain:
        changes.append(f"fd {list(delta.before.fd_chain)!r} -> {list(delta.after.fd_chain)!r}")
    if delta.before.bk_chain != delta.after.bk_chain:
        changes.append(f"bk {delta.before.bk_chain!r} -> {delta.after.bk_chain!r}")
    if delta.before.count != delta.after.count:
        changes.append(f"count {delta.before.count!r} -> {delta.after.count!r}")
    if delta.before.is_corrupted != delta.after.is_corrupted:
        changes.append(
            f"corrupted {delta.before.is_corrupted!r} -> {delta.after.is_corrupted!r}"
        )
    return message.notice(f"* {prefix}: " + ", ".join(changes))
...
snapshot_parser = argparse.ArgumentParser(
    description="Capture a reusable snapshot of the current ptmalloc heap state."
)
snapshot_parser.add_argument("name", nargs="?", default=None, help="Snapshot name.")
snapshot_parser.add_argument(
    ...
    type=int,
    default=None,
    help="Arena address to snapshot. Defaults to the current thread arena.",
)
snapshot_parser.add_argument(
    "-t",
    "--tcache",
    ...
    type=int,
    default=None,
    help="Optional explicit tcache address.",
)
snapshot_parser.add_argument(
    "-c",
    "--count",
    ...
    type=int,
    default=None,
    help="Optional maximum number of chunks to snapshot.",
)


@pwndbg.commands.Command(snapshot_parser, category=CommandCategory.PTMALLOC2)
@pwndbg.commands.OnlyWithResolvedHeapSyms
@pwndbg.commands.OnlyWhenHeapIsInitialized
@pwndbg.commands.OnlyWhenUserspace
def heap_snapshot(
    ...
    if name is None:
        name = f"snap-{len(list_heap_snapshots()) + 1}"

    snapshot = capture_heap_state_snapshot(
        name,
        allocator,
        arena_addr=arena_addr,
        tcache_addr=tcache_addr,
        max_chunks=max_chunks,
    )
    store_heap_snapshot(snapshot)

    print(
        message.success(
            f"Stored heap snapshot {name!r} ({snapshot.chunk_count} chunks, arena={snapshot.arena_addr:#x})"
        )
    )
    for note in snapshot.notes:
        print(message.warn(note))


snapshots_parser = argparse.ArgumentParser(description="List captured heap snapshots.")


@pwndbg.commands.Command(snapshots_parser, category=CommandCategory.PTMALLOC2)
@pwndbg.commands.OnlyWhenUserspace
def heap_snapshots() -> None:
    names = list_heap_snapshots()
    if not names:
        print(message.notice("No heap snapshots captured yet."))
        return
    for idx, name in enumerate(names, start=1):
        print(f"{idx:2d}. {name}")


diff_parser = argparse.ArgumentParser(description="Diff two captured heap snapshots.")
diff_parser.add_argument("before", nargs="?", default=None, help="Older snapshot name.")
diff_parser.add_argument("after", nargs="?", default=None, help="Newer snapshot name.")
diff_parser.add_argument(
    "--max-chunks",
    type=int,
    default=20,
    help="Maximum number of chunk deltas to print.",
)
diff_parser.add_argument(
    "--max-bins",
    type=int,
    default=20,
    help="Maximum number of bin deltas to print.",
)


@pwndbg.commands.Command(diff_parser, category=CommandCategory.PTMALLOC2)
@pwndbg.commands.OnlyWhenUserspace
def heap_diff(
    ...
    if before is None and after is None:
        if len(names) < 2:
            print(message.error("Need at least two snapshots. Use `heap-snapshot` first."))
            return
        before, after = names[-2], names[-1]
    elif after is None:
        if not names:
            print(message.error("No snapshots captured yet."))
            return
        after = names[-1]

    assert before is not None and after is not None
    before_snapshot = get_heap_snapshot(before)
    after_snapshot = get_heap_snapshot(after)
    if before_snapshot is None:
        print(message.error(f"Unknown snapshot: {before!r}"))
        return
    if after_snapshot is None:
        print(message.error(f"Unknown snapshot: {after!r}"))
        return

    diff = diff_heap_snapshots(before_snapshot, after_snapshot)
    print(ctx_color.banner(f"heap diff: {before} -> {after}"))

    if diff.changed_fields:
        print(message.hint("metadata"))
        for field_name, (old, new) in diff.changed_fields.items():
            print(
                "  "
                + message.notice(
                    f"{_format_snapshot_field(field_name, old)} -> {_format_snapshot_field(field_name, new)}"
                )
            )

    if diff.chunk_deltas:
        print(message.hint("chunks"))
        for delta in diff.chunk_deltas[: max(0, max_chunks)]:
            print("  " + _format_chunk_delta(delta))
        if len(diff.chunk_deltas) > max_chunks:
            print(message.warn(f"  ... {len(diff.chunk_deltas) - max_chunks} more chunk deltas"))

    if diff.bin_deltas:
        print(message.hint("bins"))
        for delta in diff.bin_deltas[: max(0, max_bins)]:
            print("  " + _format_bin_delta(delta))
        if len(diff.bin_deltas) > max_bins:
            print(message.warn(f"  ... {len(diff.bin_deltas) - max_bins} more bin deltas"))

    if not diff.changed_fields and not diff.chunk_deltas and not diff.bin_deltas:
        print(message.success("No heap changes detected between snapshots."))
