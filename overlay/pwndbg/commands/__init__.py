...
import pwndbg.aglib
import pwndbg.aglib.container_rootfs
...
from pwndbg.aglib.heap.ptmalloc import populate_global_heap_configs_from_allocator
...
def OnlyWithResolvedHeapSyms(function: Callable[P, T]) -> Callable[P, T | None]:
    ...
    def _OnlyWithResolvedHeapSyms(*a: P.args, **kw: P.kwargs) -> T | None:
        ...
        pwndbg.aglib.container_rootfs.ensure_gdb_container_rootfs_preflight()
        ...
        if (
            ...
            if isinstance(pwndbg.aglib.heap.current, HeuristicHeap):
                populate_global_heap_configs_from_allocator(pwndbg.aglib.heap.current)
                log.debug("heap resolution path: heuristic")
            else:
                log.debug("heap resolution path: debug-symbols")
        ...
        if (
            ...
            if heuristic_heap.can_be_resolved():
                ...
                populate_global_heap_configs_from_allocator(heuristic_heap)
                log.debug("heap resolution path: heuristic")
                w(
