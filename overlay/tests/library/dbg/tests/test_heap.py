...
@pwndbg_test
async def test_heap_command_uses_stock_chunk_printer(ctrl: Controller) -> None:
    await launch_to(ctrl, HEAP_MALLOC_CHUNK, "break_here")

    heap_output = await ctrl.execute_and_capture("heap allocated_chunk --count 1")
    malloc_chunk_output = await ctrl.execute_and_capture("malloc-chunk allocated_chunk")

    assert heap_output.splitlines() == malloc_chunk_output.splitlines()
